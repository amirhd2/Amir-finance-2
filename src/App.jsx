...
const getLoanNextDueInfo = (loan, txList) => {
  ...
  const nextDueDateIso = dueInfo.year && dueInfo.monthIdx !== undefined && dueInfo.day
    ? `${dueInfo.year}/${String(dueInfo.monthIdx + 1).padStart(2, '0')}/${String(dueInfo.day).padStart(2, '0')}`
    : dueInfo.dateStr;
  return {
    nextDueNum,
...
function VirtualNumericKeyboard({ isOpen, onClose, onKeyPress, isDark }) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
...
function App() {
    ...
    const handleFocusIn = (e) => {
      ...
      if (target && target.tagName === 'INPUT') {
        ...
        const isNumeric = 
          ...
          target.name?.includes('iban') ||
          target.name?.includes('year') ||
          target.name?.includes('day') ||
          target.id?.includes('amount') ||
          target.id?.includes('price') ||
          target.id?.includes('phone') ||
          target.id?.includes('card') ||
          target.id?.includes('year') ||
          target.placeholder?.includes('مبلغ') ||
          target.placeholder?.includes('تومان') ||
          target.placeholder?.includes('شماره') ||
          target.placeholder?.includes('09') ||
          target.placeholder?.includes('IR');
  ...
  // Ensure stack wizard never causes card or page jump on mobile keyboard appearance
  React.useEffect(() => {
    setPeekAnim(false);
    if (showStackWizard) {
      const lockScroll = () => {
  ...
  const handleRefreshData = async tabName => {
    try {
      ...
      if (typeof window !== 'undefined' && window.GoogleDriveSync && typeof window.GoogleDriveSync.forceSync === 'function') {
        try {
          syncResult = await window.GoogleDriveSync.forceSync({ isUserInitiated: true });
        } catch (syncErr) {
          ...
          if (dueToday.length > 0) {
            ...
            if (lastNotified !== todayIso) {
              ...
              if (navigator.serviceWorker && navigator.serviceWorker.ready) {
                ...
                try { new Notification(title, { body, icon: './icon-192x192.png' }); } catch(e) {}
              }
  ...
  const handleConfirmDeleteAllData = () => {
    ...
    const keysToRemove = [
      'amir_fin_contacts_v3', 'amir_fin_loans_v3', 'amir_fin_txs_v3', 'amir_fin_completed_periods_v3',
      'amir_fin_contacts_v2', 'amir_fin_loans_v2', 'amir_fin_txs_v2', 'amir_fin_completed_periods_v2',
      'amir_fin_contacts', 'amir_fin_loans', 'amir_fin_tx', 'amir_fin_completed_periods',
      'amir_fin_reminders', 'amir_fin_reminders_v1', 'amir_fin_reminders_v2', 'amir_fin_reminders_v3'
    ];
    ...
    if (window.GoogleDriveSync && window.GoogleDriveSync.isSignedIn()) {
      try {
        window.GoogleDriveSync.forceSync({ isUserInitiated: true });
      } catch (e) {}
    }
  ...
  useEffect(() => {
    ...
    return () => { window.showAppToast = null; };
  }, []);
  ...
  const contactWizardCards = [{
    ...
    React.createElement("input", {
      ...
      className: "w-full bg-[#F4F7FC] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-xs text-slate-900 dark:text-white placeholder:text-slate-400/80 dark:placeholder:text-slate-500/80 placeholder:font-normal focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
    })))
  }, {
    id: 'contact_info',
    ...
    React.createElement("input", {
      type: "tel",
      inputMode: "tel",
      dir: contactWizardForm.phone ? "ltr" : "rtl",
    ...
    React.createElement("input", {
      type: "text",
      inputMode: "numeric",
      dir: contactWizardForm.bankCard ? "ltr" : "rtl",
    ...
    /*#__PURE__*/
    React.createElement("div", {
      className: "relative flex items-center w-full bg-[#F4F7FC] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 transition-all",
      dir: "ltr"
    }, /*#__PURE__*/
    React.createElement("span", {
      className: "px-3.5 py-3 text-xs font-bold text-slate-500 dark:text-slate-400 font-mono tracking-wider select-none bg-slate-200/60 dark:bg-slate-800/80 border-r border-slate-200 dark:border-slate-700 shrink-0 flex items-center justify-center pointer-events-none"
    }, "IR"), /*#__PURE__*/
    React.createElement("input", {
      type: "text",
      inputMode: "numeric",
      dir: contactWizardForm.iban && contactWizardForm.iban.replace(/^IR/i, '').trim() ? "ltr" : "rtl",
      placeholder: "شماره شبا را وارد کنید",
      value: contactWizardForm.iban ? contactWizardForm.iban.replace(/^IR/i, '') : '',
      onChange: e => {
        const raw = toEnglishDigits(e.target.value).replace(/[^0-9]/g, '').slice(0, 24);
        setContactWizardForm(prev => ({
          ...prev,
          iban: raw ? `IR${raw}` : ''
        }));
      },
      className: "w-full bg-transparent p-3 text-xs text-slate-900 dark:text-white placeholder:text-slate-400/80 dark:placeholder:text-slate-500/80 placeholder:font-normal font-mono focus:outline-none transition-all"
    }))))
  }];
          ...
          React.createElement("div", {
            ...
            React.createElement("div", {
              ...
              React.createElement("p", {
                ...
                ? "سیستم اعلان فعال است. اقساط سررسید شده هر روز ساعت ۱۰:۰۰ صبح به صورت اعلان روی گوشی شما ارسال می‌شوند."
                : notifPermission === 'denied'
                ? "دسترسی به اعلان‌ها مسدود شده است. لطفاً از بخش تنظیمات سایت/مرورگر، دسترسی Notifications را مجاز کنید."
                : "با فعال‌سازی این بخش، موعد پرداخت اقساط هر روز در ساعت ۱۰:۰۰ صبح به عنوان اعلان یادآوری می‌شود.")
            )
          ),
          notifPermission === 'granted'
            ? React.createElement("div", {
                ...
                React.createElement("button", {
                  ...
                  isSendingTestNotif ? "در حال ارسال..." : "ارسال اعلان آزمایشی (تست)"
                ),
                React.createElement("button", {
                  type: "button",
                  onClick: async () => {
            ...
            : React.createElement("button", {
                ...
                "فعال‌سازی اعلان هوشمند اقساط (ساعت ۱۰:۰۰)"
              )
        )
        ))), /*#__PURE__*/
        /*#__PURE__*/
            ...
            React.createElement("div", {
              ...
              React.createElement("div", {
                ...
                React.createElement("div", {
                  ...
                  React.createElement("p", {
                    className: "text-xs font-bold text-slate-900 dark:text-white"
                  }, "همگام‌سازی با گوگل درایو"),
                  gdriveSyncState.isSignedIn && React.createElement("span", {
                ...
                }, gdriveSyncState.isSignedIn ? (gdriveSyncState.lastSyncTime ? ("آخرین همگام‌سازی: " + (window.GoogleDriveSync ? window.GoogleDriveSync.getLastSyncFormatted() : "به‌روز")) : "همگام‌سازی خودکار در پس‌زمینه فعال است") : "پشتیبان‌گیری ابری و امن اطلاعات مالی در Google Drive")
              )
            )
          ),
          React.createElement("div", {
              ...
              React.createElement("button", {
                ...
                onClick: async e => {
                  ...
                  if (window.GoogleDriveSync) {
                    const res = await window.GoogleDriveSync.forceSync({ isUserInitiated: true });
                    if (res && res.success) {
                ...
                gdriveSyncState.isSyncing ? "در حال سینک..." : "سینک دستی"
              ),
              React.createElement("button", {
                type: "button",
                onClick: e => {
      ...
      case 'all-transactions':
        {
          ...
          const filteredAllTxs = transactions.filter(tx => {
            ...
            if (allTxsSearchQuery && allTxsSearchQuery.trim()) {
              ...
              const matches = titleStr.includes(q) || notesStr.includes(q) || contactName.includes(q) || loanTitle.includes(q) || amtStr.includes(q);
  ...
  React.createElement("input", {
    ...
    className: "w-full bg-[#F4F7FC] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-xs placeholder:text-slate-400/80 dark:placeholder:text-slate-500/80 placeholder:font-normal"
  }), /*#__PURE__*/
  /*#__PURE__*/
  React.createElement("input", {
    type: "text",
  ...
  React.createElement("input", {
    type: "tel",
    inputMode: "tel",
    dir: newContactForm.phone ? "ltr" : "rtl",
    placeholder: "شماره موبایل را وارد کنید",
    ...
    className: "w-full bg-[#F4F7FC] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-xs placeholder:text-slate-400/80 dark:placeholder:text-slate-500/80 placeholder:font-normal font-mono"
  }), /*#__PURE__*/
  /*#__PURE__*/
  React.createElement("input", {
    type: "text",
    placeholder: "نام بانک / موسسه (اختیاری)",
  ...
  React.createElement("input", {
    type: "text",
    inputMode: "numeric",
    dir: newContactForm.bankCard ? "ltr" : "rtl",
    placeholder: "شماره کارت را وارد کنید",
  ...
  React.createElement("span", {
    className: "px-3 py-2.5 text-xs font-bold text-slate-500 dark:text-slate-400 font-mono tracking-wider select-none bg-slate-200/60 dark:bg-slate-800/80 border-r border-slate-200 dark:border-slate-700 shrink-0 flex items-center justify-center pointer-events-none"
  }, "IR"), /*#__PURE__*/
  React.createElement("input", {
    type: "text",
    inputMode: "numeric",
    dir: newContactForm.iban && newContactForm.iban.replace(/^IR/i, '').trim() ? "ltr" : "rtl",
    placeholder: "شماره شبا را وارد کنید",
    value: newContactForm.iban ? newContactForm.iban.replace(/^IR/i, '') : '',
    onChange: e => {
      const raw = toEnglishDigits(e.target.value).replace(/[^0-9]/g, '').slice(0, 24);
      setNewContactForm({
        ...newContactForm,
    ...
    className: "w-full bg-transparent p-2.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400/80 dark:placeholder:text-slate-500/80 placeholder:font-normal font-mono focus:outline-none"
  })), /*#__PURE__*/
  /*#__PURE__*/
  React.createElement("div", {
  ...
  React.createElement("input", {
    type: "tel",
    inputMode: "tel",
    dir: editContactForm.phone ? "ltr" : "rtl",
    placeholder: "شماره موبایل را وارد کنید",
  ...
  React.createElement("input", {
    type: "text",
    inputMode: "numeric",
    dir: editContactForm.bankCard ? "ltr" : "rtl",
    placeholder: "شماره کارت را وارد کنید",
  ...
  React.createElement("input", {
    type: "text",
    inputMode: "numeric",
    dir: editContactForm.iban && editContactForm.iban.replace(/^IR/i, '').trim() ? "ltr" : "rtl",
    placeholder: "شماره شبا را وارد کنید",
    value: editContactForm.iban ? editContactForm.iban.replace(/^IR/i, '') : '',
    onChange: e => {
      const raw = toEnglishDigits(e.target.value).replace(/[^0-9]/g, '').slice(0, 24);
      setEditContactForm({
        ...editContactForm,
